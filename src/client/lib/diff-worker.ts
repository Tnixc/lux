...
function decodeEntityToken(token: string) {
  ...
  return namedEntities[token.toLowerCase()] ?? null
}
    ...
    createElement() {
      ...
      return {
        ...
        set innerHTML(value: string) {
          inner = value
          text = value.replace(/&(#x?[0-9a-fA-F]+|[a-zA-Z][a-zA-Z0-9]+);/g, (match, token) => {
            return decodeEntityToken(token) ?? match
          })
        },
        get textContent() {
          return text
      ...
      } as HtmlEntityDecoderElement
    }
  }
}

let refractor: (typeof import('refractor/all'))['refractor'] | null = null
let refractorInitPromise: Promise<void> | null = null

function ensureRefractorLoaded() {
  if (refractor) {
    return Promise.resolve()
  }

  if (!refractorInitPromise) {
    refractorInitPromise = import('refractor/all').then((module) => {
      refractor = module.refractor
    })
  }

  return refractorInitPromise
}
...
function highlight(code: string, lang: string): string {
  if (!refractor) return escapeHtml(code)
...
function highlightFileByLines(content: string, lang: string): string[] {
  if (!content) return []
  if (!refractor) return content.split('\n').map(escapeHtml)
...
${patch}`
  ...
  const prevLanguage = previousFilename ? guessLang(previousFilename) : language
  ...
  const oldHighlightedLines = oldContent ? highlightFileByLines(oldContent, prevLanguage) : null
  const newHighlightedLines = newContent ? highlightFileByLines(newContent, language) : null
  ...
  const hunks: (DiffHunk | DiffSkipBlock)[] = rawHunks.map((hunk) => {
    ...
    return {
      ...
      lines: hunk.lines.map((line): DiffLine => {
        ...
        const singleSegmentIsNormal = hasSingleSegment && line.content[0].type === 'normal'
        ...
        return {
          ...
          content: line.content.map((seg) => {
            ...
            if (singleSegmentIsNormal) {
              ...
              if (line.type === 'delete' && oldHighlightedLines && oldNum !== undefined) {
                html = oldHighlightedLines[oldNum - 1] ?? highlight(seg.value, prevLanguage)
              } else if (line.type === 'insert' && newHighlightedLines && newNum !== undefined) {
                html = newHighlightedLines[newNum - 1] ?? highlight(seg.value, language)
              } else if (line.type === 'normal' && newHighlightedLines && newNum !== undefined) {
...
function highlightFileLines(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const highlighted = highlightedLines[lineNum - 1] ?? highlight(lineContent, language)
...
self.onmessage = async (event: MessageEvent<WorkerRequest>) => {
  ...
  try {
    await ensureRefractorLoaded().catch(() => {
      // Continue without syntax highlighting if refractor fails to load.
    })
